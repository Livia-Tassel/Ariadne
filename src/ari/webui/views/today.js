...
import { fmt, fmtDeviation, leadMetric } from "../lib/fmt.js";
import { hitRate, recentDeviations, store, todos } from "../lib/store.js";
...
function movement(run) {
  ...
  const point = Array.isArray(predicted) ? `${fmt(predicted[0])}~${fmt(predicted[1])}` : fmt(predicted);
  const deviation = judgement ? fmtDeviation(judgement.deviation) : "";
  return html`<span class="move"
    >${point} → <b>${fmt(agg.mean)}</b>${deviation && deviation !== "—" ? html`　<b>${deviation}</b>` : ""}</span
  >`;
}
...
function strip() {
  const rows = recentDeviations();
  if (!rows.length) return "";
  return html`<p class="spread-strip">
    <span class="lbl">最近的落差</span>
    ${rows.map(
      (row) => html`<s
        class="${row.hot ? "hot" : ""}"
        title="${row.batch} ${row.run} · ${row.metric}"
