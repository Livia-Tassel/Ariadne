...
`ari` 是一个陪伴科研人员的本地实验工作台。它把**实验开跑前先写下对结果的预测**这件事变成低摩擦的流程，再让结果与预测的差异自动浮上来，逼出真正的反思。v0.5 把工作流向前延伸到领域调研：从可核实的论文清单一路走到瓶颈、想法与实验。
...
摘要不会在抓取后自动跑，避免一次调研突然发出几十次付费请求。每篇长尾右侧都有「AI 摘要」按钮；需要批量处理时，可用标题栏的「批量 AI 摘要」一次处理最多 12 篇。再次点击会继续尚未摘要的论文，不会重复消耗已经生成过的内容。
...
在实验闭环里，它做两件事，CLI 与 GUI 共用同一个入口（`advising.advise` / `probing.probe`），两边的降级行为不会漂移；领域调研另外提供检索式建议、逐篇长尾摘要与独立瓶颈判断：
